...
class CommandBus {
  ...
  /// 关联的 QueryBus，用于命令执行后自动清除查询缓存
  QueryBus? _queryBus;

  /// 连接 QueryBus，使命令执行成功后自动清除相关查询缓存
  void connectQueryBus(QueryBus queryBus) {
    _queryBus = queryBus;
  }
  ...
  Future<CommandResult<T>> dispatch<T>(Command<T> command) async {
    ...
    try {
      ...
      // 合并 CommandResult 中的事件和 CommandContext 中的待发布事件（去重）
      ...
      // 缓存失效：命令执行成功后自动清除相关查询缓存
      if (_queryBus != null && result.isSuccess) {
        if (command is CommandWithCacheInvalidation) {
          (command as CommandWithCacheInvalidation).cacheInvalidationKeys.forEach(_queryBus!.clearCache);
        } else {
          _queryBus!.clearCache();
        }
      }
  ...
  Future<void> undo(Command command) async {
    ...
    try {
      ...
      _eventStreamController.add(CommandUndoneEvent(command: command));
    } catch (e, stackTrace) {
      _commandStreamController.add(
        CommandUndoFailed(
          command: command,
