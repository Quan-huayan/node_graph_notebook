...
/// 获取邻居节点的Handler
///
/// 使用GraphRepository内部的邻接表优化查询，提供O(1)复杂度的邻居查询
class GetNeighborNodesQueryHandler extends QueryHandler<List<Node>, GetNeighborNodesQuery> {
  ...
  Future<QueryResult<List<Node>>> handle(GetNeighborNodesQuery query) async {
    try {
      // 加载中心节点
      final centerNode = await _nodeRepository.load(query.nodeId);
      if (centerNode == null) {
        return QueryResult.failure('Center node not found: ${query.nodeId}');
      }

      // 使用GraphRepository获取邻居ID
      final neighborIds = <String>{};

      if (query.direction == NeighborDirection.outgoing ||
          query.direction == NeighborDirection.both) {
        neighborIds.addAll(_graphRepository.getOutgoingNeighbors(query.nodeId));
      }

      if (query.direction == NeighborDirection.incoming ||
          query.direction == NeighborDirection.both) {
        neighborIds.addAll(_graphRepository.getIncomingNeighbors(query.nodeId));
      }
      ...
      final neighbors = await _nodeRepository.loadAll(neighborIds.toList());
...
/// 获取入边引用的Handler
///
/// 使用GraphRepository内部的邻接表优化查询，提供O(1)复杂度的入边查询
class GetIncomingReferencesQueryHandler extends QueryHandler<List<Node>, GetIncomingReferencesQuery> {
  ...
  Future<QueryResult<List<Node>>> handle(GetIncomingReferencesQuery query) async {
    try {
      // 使用GraphRepository获取入边邻居ID
      final incomingIds = _graphRepository.getIncomingNeighbors(query.nodeId);
      ...
      // 批量加载节点
      final incomingNodes = await _nodeRepository.loadAll(incomingIds.toList());
...
/// 获取节点度数的Handler
///
/// 使用GraphRepository内部的邻接表优化查询，提供O(1)复杂度的度数查询
class GetNodeDegreeQueryHandler extends QueryHandler<NodeDegree, GetNodeDegreeQuery> {
  ...
  Future<QueryResult<NodeDegree>> handle(GetNodeDegreeQuery query) async {
    try {
      final node = await _nodeRepository.load(query.nodeId);
      if (node == null) {
        return QueryResult.failure('Node not found: ${query.nodeId}');
      }

      // 使用GraphRepository获取度数
      final (inDegree, outDegree) = _graphRepository.getNodeDegree(query.nodeId);
