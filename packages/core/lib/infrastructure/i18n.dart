...
///
/// Stores metadata about a language including its code, English name, and native name.
class LanguageInfo {
  /// Creates a LanguageInfo instance.
  ///
  ...
  /// The language code (e.g., 'en' for English, 'zh' for Chinese).
  final String code;
...
/// - 支持语言包插件注册（通过 registerLanguage 方法）
class I18n extends ChangeNotifier {
  ...
  /// 检查是否支持指定语言
  bool supportsLanguage(String language) => _supportedLanguages.containsKey(language);

  /// 获取所有支持的语言代码
  List<String> get supportedLanguages => _supportedLanguages.keys.toList();

  /// 获取语言信息
  LanguageInfo? getLanguageInfo(String code) => _supportedLanguages[code];

  /// 注册语言（插件调用）
  ///
  /// [code] 语言代码，如 'en', 'zh', 'ja'
  /// [name] 英文名称，如 'English', 'Chinese', 'Japanese'
  /// [nativeName] 本地名称，如 'English', '简体中文', '日本語'
  void registerLanguage(String code, String name, String nativeName) {
    _supportedLanguages[code] = LanguageInfo(
      code: code,
      name: name,
      nativeName: nativeName,
    );
    _log.info('Language registered: $code ($nativeName)');
    notifyListeners();
  }
  ...
  Map<String, int> getTranslationStats() {
    ...
    for (final lang in supportedLanguages) {
      ...
      stats[lang] = _dynamicTranslations[lang]?.length ?? 0;
    }
