...
class MiddlewarePipeline {
  ...
  Future<QueryResult> executeQuery(
    Query query,
    QueryPipelineHandler handler,
  ) async {
    var index = 0;

    Future<QueryResult?> next(Query q) async {
      if (index < _queryMiddleware.length) {
        final middleware = _queryMiddleware[index++];
        if (middleware.canHandle(q)) {
          final result = await middleware.handle(q, next);
          if (result != null) {
            return result;
          }
        }
    ...
    return result ?? QueryResult.failure('Query returned null result');
  }
...
/// 查询管道处理器类型
typedef QueryPipelineHandler = Future<QueryResult> Function(Query query);
