...
import 'package:node_graph/bloc/graph_bloc.dart';
import 'package:node_graph/bloc/graph_event.dart';
import 'package:node_graph/bloc/node_bloc.dart';
import 'package:node_graph/bloc/node_event.dart';
import 'package:node_graph/bloc/node_state.dart';
import 'package:node_graph/command/node_commands.dart';
import 'package:node_graph/service/node_service.dart';
import 'package:provider/provider.dart';

const _log = AppLogger('FolderTreeView');
...
class _FolderTreeViewState extends State<FolderTreeView> {
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      _initialized = true;
      _log.info('didChangeDependencies: initializing...');
      
      _commandBus = context.read<CommandBus>();
      _log.info('✓ CommandBus obtained');
  ...
  Future<void> _calculateDepths() async {
    _log.info('_calculateDepths: starting...');
    
    try {
      final nodeService = context.read<NodeService>();
      _log.info('✓ NodeService obtained');
      
      final allNodes = [...widget.nodes, ...widget.folders];
      _log.info('Calculating depths for ${allNodes.length} nodes');
      
      _nodeDepths = await nodeService.calculateNodeDepths(allNodes);
      _log.info('✓ Depths calculated: ${_nodeDepths?.length} entries');
      
      setState(() {});
    } catch (e, st) {
      _log.warning('✗ Failed to calculate depths: $e');
