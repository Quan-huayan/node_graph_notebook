...
import 'package:core/cqrs/queries/load_node_query.dart';
import 'package:core/cqrs/queries/search_nodes_query.dart';
...
class NodeBloc extends Bloc<NodeEvent, NodeState> {
  ...
  Future<void> _onLoadNodes(
    ...
    try {
      // 通过 QueryBus 执行查询（读操作）
      final result = await _queryBus.dispatch<List<Node>, LoadAllNodesQuery>(
  ...
  Future<void> _onConnectNodes(
    ...
    try {
      ...
      if (result.isSuccess) {
        // 通过 QueryBus 重新加载节点以获取更新后的引用列表
        final nodeResult = await _queryBus.dispatch<Node?, LoadNodeQuery>(
          LoadNodeQuery(nodeId: event.fromNodeId),
        );
        final fromNode = nodeResult.isSuccess ? nodeResult.data : null;
        if (fromNode != null) {
