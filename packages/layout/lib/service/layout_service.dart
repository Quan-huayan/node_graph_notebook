...
import 'package:core/utils/types.dart';
...
abstract class LayoutService {
  ...
  /// 
  /// [nodes] - 要布局的节点列表
  /// [currentPositions] - 节点当前位置映射（nodeId -> Offset）
  /// [algorithm] - 布局算法
  /// [options] - 布局选项
...
class LayoutServiceImpl implements LayoutService {
  ...
  Future<void> forceDirectedLayout({
    ...
    for (final node in nodes) {
      final currentPos = currentPositions[node.id];
      if (currentPos == null || currentPos == Offset.zero) {
        ...
        positions[node.id] = Vector2(
          currentPos.dx.toDouble(),
          currentPos.dy.toDouble(),
        );
      }
    }
