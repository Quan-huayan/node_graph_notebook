...
const _log = AppLogger('LuaDynamicHookManager');
...
class LuaDynamicHookManager {
  ...
  /// Hook注册表
  final HookRoleRegistry hookRegistry;

  /// 动态Hook映射
  ///
  /// Key: Hook ID
  /// Value: Hook实例
  final Map<String, HookRoleBase> _dynamicHooks = {};
  ...
  void registerAPIs() {
    ...
    engineService..registerFunction('registerToolbarButton', (args) {
      try {
        ...
        _log.info('Error registering toolbar button: $e');
        return 0;
      }
    })

    // 注销工具栏按钮API
    ..registerFunction('unregisterToolbarButton', (args) {
      try {
        if (args.isEmpty) {
    ...
    // 列出所有动态按钮（返回按钮数量）
    ..registerFunction('listDynamicButtons', (args) {
      final buttons = _listDynamicButtons();
      // 打印按钮列表到控制台
      _log.info('Dynamic buttons list:');
    ...
    _log.info('APIs registered successfully');
  }
  ...
  Future<int> _registerToolbarButton({
    ...
    if (_dynamicHooks.containsKey(hookId)) {
      _log.info('Button $buttonId already registered, updating...');
    ...
    _log.info('Registered toolbar button: $buttonId');
  ...
  Future<int> _unregisterToolbarButton(String buttonId) async {
    ...
    if (hook != null) {
      ...
      if (hookWrapper != null && hookWrapper.isEnabled) {
        ...
        _log.info('Hook disabled successfully');
      }
    ...
    _log.info('Button $buttonId not found');
    return 0;
  }
  ...
  /// 清空所有动态Hook
  void clear() {
    _dynamicHooks.values.forEach(hookRegistry.unregisterHook);
    _dynamicHooks.clear();
    _log.info('All dynamic hooks cleared');
  }
}
...
///
/// 由Lua脚本创建的工具栏按钮
class _DynamicToolbarHook extends MainToolbarHookRole {
  /// 构造函数
  _DynamicToolbarHook({
    required this.id,
    required this.label,
