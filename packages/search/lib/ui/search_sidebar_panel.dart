...
class _SearchSidebarPanelState extends State<SearchSidebarPanel> {
  ...
  void didChangeDependencies() {
    ...
    _layoutService = Provider.of<UILayoutService>(context, listen: false);
  }
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return SearchedNodeItem(
                          ...
                          onTap: () {
                            // === 架构说明：位置获取 ===
                            // 从 UILayoutService 获取节点位置
                            // NodeAddEvent 的 position 参数约定为中心位置
                            final attachment = _layoutService?.getNodeAttachment(node.id);
                            if (attachment != null) {
                              final centerPosition = Offset(
                                attachment.localPosition.x + node.size.width / 2,
                                attachment.localPosition.y + node.size.height / 2,
                              );
