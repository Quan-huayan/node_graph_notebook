...
class _AIChatDialogState extends State<AIChatDialog> {
  ...
  void initState() {
    ...
    if (widget.enableFunctionCalling) {
      // 从 PluginContext 获取 AIToolRegistry（通过 ServiceRegistry）
      final toolRegistry = context.read<PluginContext>().get<AIToolRegistry>();
  ...
  Future<String> _handleSummarizeCommand(
    ...
    try {
      ...
      if (registry == null) {
        return '❌ Settings registry not available.';
      }
...
''';
  ...
  AIProvider _getAIProvider() {
    ...
    if (registry == null) {
      throw Exception('Settings registry not available');
    }
