...
void main() {
  group('LuaEngineService', () {
    ...
    test('执行简单脚本', () async {
      ...
      final result = await service.executeString('print("Hello, Lua!")');
    ...
    test('处理语法错误', () async {
      ...
      final result = await service.executeString('print("unclosed string)');
    ...
    test('重置引擎', () async {
      ...
      // 变量应该被清空
      final result = await service.executeString('print(x or "nil")');
    ...
    test('未初始化时执行脚本抛出异常', () async {
      expect(
        () async => await service.executeString('print("test")'),
        throwsA(isA<StateError>()),
      );
    });
  });
