...
void main() {
  group('LuaErrorHandler', () {
    test('应该解析语法错误', () {
      const errorMessage = "syntax error near 'end'";
    ...
    test('应该提取错误行号', () {
      const errorMessage = 'script:123: error message';
...
''';
      ...
      expect(context, contains('debugPrint(undefinedVariable)'));
    });

    test('应该创建增强的错误结果', () {
      const script = 'debugPrint("hello"';
    ...
    test('应该生成格式化的错误信息', () {
      ...
      expect(formatted, contains('debugPrint(x)'));
      expect(formatted, contains('📚 堆栈:'));
      expect(formatted, contains('⏱️  执行时间: 50ms'));
    });

    test('应该检测安全错误', () {
      const errorMessage = 'LuaSecurityException: 权限不足';
      final errorInfo = LuaErrorHandler.parseError(errorMessage);
