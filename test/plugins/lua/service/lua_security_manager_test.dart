...
void main() {
  group('LuaSecurityManager', () {
    ...
    test('应该检查执行时间', () {
      const shortTime = Duration(seconds: 1);
      const longTime = Duration(seconds: 10);
  ...
  group('LuaSandboxConfig', () {
    ...
    test('默认配置应该是安全的', () {
      const defaultConfig = LuaSandboxConfig();
  ...
  group('LuaSecurityException', () {
    test('应该包含错误信息', () {
      const exception = LuaSecurityException('Test error');
    ...
    test('应该包含安全原因', () {
      const exception = LuaSecurityException(
        'Test error',
