  ...
  void _registerNodeAPIs() {
    ...
    engineService.registerFunction('createNode', (args) {
      try {
        // 🔒 参数验证
        if (args.isEmpty) {
          throw LuaArgumentException('createNode requires at least title parameter');
        }

        final title = _validateString(args[0], 'title');
        if (title == null || title.isEmpty) {
          throw LuaArgumentException('title cannot be empty');
        }
    ...
    engineService.registerFunction('updateNode', (args) {
      try {
        // 🔒 参数验证
        if (args.isEmpty) {
          throw LuaArgumentException('updateNode requires id parameter');
        }

        final id = _validateString(args[0], 'id');
        if (id == null || id.isEmpty) {
          throw LuaArgumentException('id cannot be empty');
        }
    ...
    engineService.registerFunction('deleteNode', (args) {
      try {
        // 🔒 参数验证
        if (args.isEmpty) {
          throw LuaArgumentException('deleteNode requires id parameter');
        }
    ...
    engineService.registerFunction('getNode', (args) {
      try {
        // 🔒 参数验证
        if (args.isEmpty) {
          throw LuaArgumentException('getNode requires id parameter');
        }
    ...
    engineService.registerFunction('getChildNodes', (args) {
      try {
        // 🔒 参数验证
        if (args.isEmpty) {
          throw LuaArgumentException('getChildNodes requires parentId parameter');
        }

        final parentId = _validateString(args[0], 'parentId');
        if (parentId == null || parentId.isEmpty) {
          throw LuaArgumentException('parentId cannot be empty');
        }
