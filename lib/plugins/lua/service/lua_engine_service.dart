...
///
/// // 执行脚本字符串
/// final result = await engine.executeString('debugPrint("Hello, Lua!")');
/// if (result.success) {
///   debugPrint(result.output); // ['Hello, Lua!']
/// }
///
/// // 执行脚本文件
...
class LuaEngineService {
  ...
  /// ```dart
  /// // 简单执行
  /// final result = await engine.executeString('debugPrint("Hello")');
  /// debugPrint(result.output); // ['Hello']
  ///
  /// // 带上下文变量
  /// final result2 = await engine.executeString(
  ///   'debugPrint(name .. " is " .. age .. " years old")',
  ///   context: {'name': 'Alice', 'age': 30},
  /// );
  /// debugPrint(result2.output); // ['Alice is 30 years old']
  /// ```
  ...
  ///   debugPrint('脚本执行成功');
  /// } else {
  ///   debugPrint('脚本执行失败: ${result.error}');
  /// }
  /// ```
  ...
  ///
  /// // x变量已被清空
  /// final result = await engine.executeString('debugPrint(x or "nil")');
  /// debugPrint(result.output); // ['nil']
  /// ```
  ...
  /// //   debugPrint("Callback received: " .. result)
  /// // end
  ///
  /// // 从Dart调用
