...
class LuaSandboxConfig {
  ...
  /// 创建宽松配置（开发环境）
  factory LuaSandboxConfig.permissive() {
    return const LuaSandboxConfig(
  ...
  /// 创建严格配置（生产环境）
  factory LuaSandboxConfig.strict() {
    return const LuaSandboxConfig(
...
class LuaScriptValidationResult {
  ...
  /// 创建成功结果
  factory LuaScriptValidationResult.success() {
    return const LuaScriptValidationResult(
      isValid: true,
      errors: [],
      warnings: [],
    );
  ...
  /// 创建失败结果
  factory LuaScriptValidationResult.failure(List<String> errors) {
