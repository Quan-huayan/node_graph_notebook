...
class LuaScriptService {
  ...
  ///
  /// [scriptId] 脚本ID
  LuaScript? getScriptInfo(String scriptId) => _scriptCache[scriptId];

  /// 获取所有启用的脚本
  List<LuaScript> getEnabledScripts() => _scriptCache.values.where((script) => script.enabled).toList();

  /// 获取所有脚本
  List<LuaScript> getAllScripts() => _scriptCache.values.toList();
